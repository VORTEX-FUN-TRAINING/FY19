...
- { }などのカッコでなく、インデントでループや条件分岐のネストを表現する
...
- 動作速度は十分高速だが、JavaやCと比較するとかなり遅い（5~10分の1以上）とされる。ただし、静的型付けをするCythonではCに匹敵する速度が出せるとされる。
...
基本3構造
==
- どんな複雑なプログラムも、以下の3つの構造が組み合わさて動いている
## 順次処理
- 上から順番に処理を実行する
...
> 余談ですが、Pythonでも、静的型付けの仕組みをもつCython（CPythonではない）は非常に高速に動作します。
...
float = 123.0
```
- なおPython3のint型は64ビット以上の桁も扱える
---

値のまとまりを扱う型
==
- 1つの変数で1度に沢山の値を扱いたい場合は以下の型を使う
