...
### 世界最大規模のオープンソースプロジェクトを支える
- Linuxカーネルのソースコードには、毎週1000を超えるファイルに対し2万箇所以上の変更が、数万人の手によって加えられる。gitがこれを支えている
- 世のほとんどのオープンソースプロジェクトのソースコードは、gitで管理されている
- ドキュメントの管理能力が非常に高いので、出版社とライターが原稿のやり取りに使ったり、最近では、弁護士向けにword専用のgitサーバSaaSを提供している会社もある
...
- 分散型でないVCSでは、コミット＝サーバ上のリポジトリにコミットすること
#### 何が困るか
- saveしたいだけなのに、他の作業者たちに公開される場所にコードを置かないといけない
...
重要キーワード：ブランチ (branch)
==
### コミットに付けられた別名・・・というとややこしいが、リポジトリに「パラレルワールド」を作ること
> もっとややこしい？ｗ
- ソフトウェアの開発や維持保守は複数の作業を同時に進められるのが望ましい
  - あるチームは新機能の追加
  - 別のチームは、バグフィックス、などなど
- 全部一人で作業、あるいは一つの作業を一人で担当であれば、ローカルリポジトリで作業→リモートにアップロード、だけでなんとか回りそうだが・・・
- 複数人で作業しているときに、コミットがどんどん重なっていくのはなかなか辛い
### リポジトリの中にパラレルワールドを作って、独立したらええやん＝ブランチ
- あるコミットから先を別のブランチに分け、それぞれの作業のコミットを追加していく
- ブランチでの作業が完了したら、他のブランチと合体させる
...
- 他のブランチと合体して作業を完了させることを**マージ**という
...
#### github
- 世界最大のリモートリポジトリを提供するSaaS。世の多くのOSSがここで管理されている
- リモートリポジトリの他、コードレビュー機能、課題管理、進捗管理、CI/CDなど開発に必要な多くの機能を提供する
- オープンソースであれば利用は無料。クローズドで利用したい場合、開発メンバーが4人を超えると有料となる
#### gitlab
- githubのクローンで、SaaSを提供している。パッケージが提供されているので、自前のサーバで動かすこともできる
- github以上の機能を常に提供し続けている（新機能のリリースはgitlabのほうが早く、豊富）
- クローズドPJであっても、無制限で無料で利用できる
- githubのシンプルなUIに比べてやや複雑で、好みのわかれるところ
