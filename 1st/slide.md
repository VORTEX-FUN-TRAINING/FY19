...
gitとは
==
- 分散ソースコード管理システム(分散SCM: Distributed Source Code Management system) 
	> 何が分散かは、後程解説いたします
- 誰が、いつ、どのファイルの、どの行に、どんな変更を加えたかの履歴を管理する
- Linuxカーネルのソース管理のために、リーナス御大自らが開発
	- Linuxカーネルの開発者は世界に数万人いて、毎週1000を超えるファイルに2万箇所以上の変更が加えられる。ファイルサーバでは運用不可能
	- git開発のために、リーナス氏は10日間雲隠れしたのこと。名称は、３文字で、過去に使われてないものなら何でも良かったらしい
- SCMには、他にSubversionやMercuralなどがあるが、オープンソースプロジェクトにおいてgitは事実上の標準となっている
- クライアント・サーバモデルであり、作業端末上ではgitクライアントを、サーバはwebブラウザを使って利用する
...
- リモートリポジトリをローカルに丸ごとコピーすることを**クローン**と呼ぶ
- リモートリポジトリに自分の作業を登録することを**プッシュ**と呼ぶ
- リモートリポジトリから、他の作業者の変更を取り込むことを**プル**と呼ぶ
> クローン、プッシュ、プルの詳細はいずれ説明いたします
...
- 分散でないSCMは、自分の変更を
### 分散SCMと呼ばれる所以
- リポジトリを、ローカルとリモートに分散して置けるのがgitの特徴
- 
