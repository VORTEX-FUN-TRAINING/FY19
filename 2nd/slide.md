...
### クラスとは、インスタンスの設計図で、いわばたい焼きの型のようなものです。インスタンスは、クラスから作られる、たい焼きなのです！！（ドヤァ）
##### いやいや、クラスからインスタンスができるのは、それはわかるんだけど・・・
- そもそもなぜクラスというものが必要なのかがわからない
- 変更しやすいプログラムを作ることと、たい焼きが結びつかない
#### やっぱりオブジェクト指向ってよくわかんない・・・
## 大丈夫です。なぜオブジェクト指向が必要なのか、なぜ変更しやすいプログラムが作れるのかを、これから解説いたします
...
これぞ「ポリモーフィズム（多態性）」
==
- 目的（＝得たい結果）は同じだが中身（＝処理方法）の違う関数を、同じ名前で定義する。
...
- システムは、要素が疎結合であるほど、改修がしやすい

---
オブジェクト指向３大要素を再度、振り返る
==
## 継承
- 既存のクラスの機能を拡張して、新しいクラスを作る。
## ポリモーフィズム
...
クラス・インスタンス・オブジェクトまとめ
==
### クラス
- データ構造のこと。あるいはデータ構造とそれに対する処理をまとめたもの
### インスタンス
- クラスのデータに具体的な値を入れて、メモリ上に配置されたもの
### オブジェクト
- Pythoにおいては、クラス、インスタンス、変数、関数など、構文や演算子以外のすべてのもの
- 一般的には、データのことを指したりクラスを指したりインスタンスを指したりする
---

変数のスコープ
==
- 変数には、利用できる範囲が決まっている＝スコープと呼ぶ
### グローバル変数
- クラスや関数じゃないところで定義された変数
### クラス変数
- クラス内でのみ有効な変数
### インスタンス変数
- インスタンス内でのみ有効な変数
### ローカル変数
- 関数やメソッド内で定義され、その中でのみ有効な変数
...
==
### グローバル変数について補足
- 関数やクラス内で変数が見つからない場合、グローバルスコープを参照しにいく
  - たまたま同じ名前の変数がグローバルにあり、それに値を入れようとした場合、
    - イミュータブル（文字列、数値、タプルなど）な場合、**同じ名前のローカル変数が作られる**
    - ミュータブル（リスト、辞書、集合）な場合、グローバル変数を書き換える
- グローバルスコープが有効なのは、モジュール（＝ファイル）の中のみ
  - エラーメッセージなど、定数をまとめたモジュールを作った場合は明示的にインポートする必要あり
#### グローバル変数の利用には注意しましょう
...
Pythonでのクラスの使いみち
==
- Pythonはクラスを書かなくても実行可能コードが書けますが・・・使いみちによってはクラスを書いたほうがよいです。以下は実例です
##### ファイルを読み込んでそれを保持するクラスhoge
- hoge.filelineとかで、他のクラスから利用できる
- ファイルのフォーマットのチェックや不正なデータを排除するメソッド持つ
##### webアクセスするためのクラス
- GETとかPOSTとか、webにアクセスするためのメソッドをまとめたクラス
- webアクセスして結果を返すだけなので、データは保持しない。よってインスタンス化せずにすぐ使える
> データを持たずにメソッドだけを提供するクラスをユーティリティクラスといいます
